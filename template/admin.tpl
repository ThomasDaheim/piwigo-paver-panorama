<div class="titrePage">
  <h2>Paver Panorama</h2>
</div>

<form method="post" action="" class="properties" id="psv-config">
<fieldset>
  <legend>{'Configuration'|translate}</legend>
  <p>{'paverpanorama_config_intro'|translate}</p>
  <br>
  <ul>
    <li>
      <label>
        <span class="graphicalCheckbox icon-check{if not $PaverPanorama.display_help}-empty{/if}"></span>
        <input type="checkbox" name="display_help"{if $PaverPanorama.display_help} checked="checked"{/if}>
    ...
    <li>
      <label>
        <span class="graphicalCheckbox icon-check{if not $PaverPanorama.auto_anim}-empty{/if}"></span>
        <input type="checkbox" name="auto_anim"{if $PaverPanorama.auto_anim} checked="checked"{/if}>
        <b>{'Automatically launch animation'|translate}</b>
      </label>
    </li>
    <li>
      <label>
        <span class="graphicalCheckbox icon-check{if not $PaverPanorama.display_icon}-empty{/if}"></span>
        <input type="checkbox" name="display_icon"{if $PaverPanorama.display_icon} checked="checked"{/if}>
    ...
    <li>
      <label>
        ...
        <input type="number" min="1024" max="8192" value="{$PaverPanorama.raw_width}" name="raw_width"> <b>px</b>
      </label>
      <br>
